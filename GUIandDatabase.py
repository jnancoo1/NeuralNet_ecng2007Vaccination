...
class MyLayout(ScreenManager):
    ...
    def on_kv_post(self, base_widget):
        ...
        self.dropdown = MDDropdownMenu(
            caller=caller,
            items=[{"viewclass": "OneLineListItem", "text": "ASTRAZENICA",
                    "on_release": lambda x=f"ASTRAZENICA": self.set_item(x)},
                   {"viewclass": "OneLineListItem", "text": "SINOPHARM",
                    "on_release": lambda x=f"SINOPHARM": self.set_item(x)},
                   {"viewclass": "OneLineListItem", "text": "PFIZER",
                    "on_release": lambda x=f"PFIZER": self.set_item(x)},
                   {"viewclass": "OneLineListItem", "text": "JOHNSON & JOHNSON",
                    "on_release": lambda x=f"JOHNSON & JOHNSON": self.set_item(x)}],
            width_mult=4
        )
    ...
    ###
    ### Function for Data Scan ###
    def capture(self):
        camera = self.ids.camera_id
        camera.export_to_png("imgcapture.jpg")
    ###
    ### Function for Stamp Scan ###
    def stamp_capture(self):
        camera = self.ids.camera_id
        camera.export_to_png("stampcap.jpg")
    ###
    ...
    def refresh(self):
        ...
        for record in records:
            image = ImageLeftWidget(source = "pic.png")
    ...
    def submit(self):
        ...
        if temp == False:
            ...
            temp = False
                ...
                month) == 10 or int(month) == 12) and (int(day) < 1 or int(day) > 31):
            toast("Invalid Date of Birth")
            self.ids.day.text = ""
            temp = False
            ...
            self.ids.day.text = ""
            temp = False
        elif (int(month) == 4 or int(month) == 6 or int(month) == 9 or int(month) == 11) and (
    ...
    def submit_confirm(self, obj):
        ...
        dbcursor.execute("SELECT First_Name, Middle_Initial, Surname, Date_Of_Birth FROM Persons WHERE "
                         "First_Name=? AND Middle_Initial=? AND Surname=? AND Date_Of_Birth=?",
                         (f_name, m_name, s_name, dob))
